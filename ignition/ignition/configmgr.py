...
class FileToParse(object):
    ...
    def GetNavPane(self,session, FilesToParse, current):
        ...
        navPane = self.FormMgr.GetHtmlTable(["Navigation"])
        
        ChangeFileTb = self.FormMgr.GetHtmlTable([self.FormMgr.GetDropdown(session, "CurrentFile", FilesToParse, current)], 'nav_ConfMgr_changeFile', 'ConfigMgr')
        ChangeFileTb.AddRow([self.FormMgr.GetButton(session, "View Selected File Config",'nav_ConfMgr_changeFile', 'body', "CurrentSelectedFile")])
    ...
    def ProcessFormInputs(self, session, returnForm=None):
        ...
        for DataPattern in self.Data['DataPattern']:
            ...
            if result is not None:
                print ("DataPattern: result was not None" %(result))
                if result[0] == 1:
                    """
                        redirect required
                    """
        ...
        if 'config wiz' in forms['ButonList'] and forms['ButonList']['config wiz'] is not None:
            if 'Data__Time__configwiz' in forms['ButonList']['config wiz']:
                print ("Need to run config wizard for time")
                return (1, 'Data__Time__configwiz', TimeConfigWiz(self.Data['Time'], self.Location+self.name))
        """
    ...
    def GetConfigHtmlView(self,session):
        ...
        self.FormMgr.reset_forms()
        
        BaseTable = self.FormMgr.GetHtmlTable([self.name])
        FindFilePattern = self.FormMgr.GetHtmlTable(['File Search Patterns'])
        ...
        """
        DataB_Time = self.FormMgr.GetHtmlTable(['Time Configuration'])
        ...
        """
        if self.Data['Time']['inLine'] == False:
             DataB_TimeSubPattern = self.FormMgr.GetHtmlTable(['Pattern', 'Config'])
             DataB_TimeSubPatternSub = self.FormMgr.GetHtmlTable(['param', 'splitConfig', 'action'])
             for patternT in self.Data['Time']['pattern']:
                 print ("PatternT: %s" %(patternT))
                 print (self.Data['Time']['pattern'])
                 ...
                 DataB_TimeSubPatternSub.AddRow(['Mon', self.FormMgr.GetTextBox(session,'Data__Time__pattern__%s__Mo'%(patternT),
                                                self.Data['Time']['pattern'][patternT]['Mo']), 
                                                self.FormMgr.GetButton(session,'test', 'Data__Time__pattern__Mo__test')])
                 DataB_TimeSubPatternSub.AddRow(['Day', self.FormMgr.GetTextBox(session,'Data__Time__pattern__%s__Dy'%(patternT),
                                                self.Data['Time']['pattern'][patternT]['Dy']), 
                                                self.FormMgr.GetButton(session,'test', 'Data__Time__pattern__Dy__test')])
                 DataB_TimeSubPatternSub.AddRow(['Hour', self.FormMgr.GetTextBox(session,'Data__Time__pattern__%s__H'%(patternT),
                                                self.Data['Time']['pattern'][patternT]['H']), 
                                                self.FormMgr.GetButton(session,'test', 'Data__Time__pattern__H__test')])
                 DataB_TimeSubPatternSub.AddRow(['Min', self.FormMgr.GetTextBox(session,'Data__Time__pattern__%s__M'%(patternT),
                                                self.Data['Time']['pattern'][patternT]['M']),
                                                self.FormMgr.GetButton(session,'test', 'Data__Time__pattern__M__test')])
    ...
    def AddDataPattern(self, pattern, tag, splitConfig):
        if not pattern == None:
            #print("AddDataPattern input: %s %s %s" %(pattern, tag, splitConfig))
            #self.Data['DataPattern'][pattern] = {'tag': '' if (tag == ('' or None)) else tag, 'unconfigured': True if ((tag or splitConfig) == ('' or None)) else False, 'splitConfig': '' if (splitConfig == ('' or None)) else splitConfig }
            self.Data['DataPattern'][pattern] = DataPattern(pattern)
        #self.ToDoList.append(self.Data['DataPattern'][pattern])
...
class DataPattern(FileToParse):
                ...
                if self.name in forms['ButonList']['add']:
                    ...
                    if 'dataPoints' in item:
                        ...
                        try:
                            ...
                            print ("ID was empty")
                            pass
                            #webInsMgr.InstanceList[int(session)].ConfigMgr.configDict[LoadedFile].runningConfig['Tool']['FilesToParse'][CurrentNavFile].Data[SplitItem[1]][SplitItem[2]].AddDataPoint(SplitItem[4], '')
                        try:
                            self.pattern[SplitItem[3]][SplitItem[4]][SplitItem[5]][forms['textBoxes'][keyId]].append(forms['textBoxes'][keyPattern])
                        except:
                            pass
                            #webInsMgr.InstanceList[int(session)].ConfigMgr.configDict[LoadedFile].runningConfig['Tool']['FilesToParse'][CurrentNavFile].Data[SplitItem[1]][SplitItem[2]].pattern[SplitItem[3]][SplitItem[4]][SplitItem[5]][NewInput[keyId]].append('')
                    else:
                        SplitItem = item.split('__')
                        if not 'tag' in item:
                            print ("New Data Pattern: Split Item is: %s " %(SplitItem))
                            #keyId = Data__DataPattern__%s__tag__new
                            #webInsMgr.InstanceList[int(session)].ConfigMgr.configDict[LoadedFile].runningConfig['Tool']['FilesToParse'][CurrentNavFile].Data[SplitItem[1]]
                        else:
    ...
    def GetHtmlOut(self,session, module=None):
        module = 'ConfigMgr' if module == None else module
        ...
        for tagId in self.pattern['tag']:
            ...
            for dataPointId in self.pattern['tag'][tagId]['dataPoints']:
                ...
                dataPointsTb.AddRow([dataPointId,spltCfg,
                                     self.FormMgr.GetHtmlTable([self.FormMgr.GetButton(session, 'test', dpDivID, dpDivID, 'Data__DataPattern__%s__tag__%s__dataPoints__%s__test'%(self.name,tagId, dataPointId)), 
                                                                self.FormMgr.GetButton(session, 'del', dpDivID, tgDpDivID, 'Data__DataPattern__%s__tag__%s__dataPoints__%s__del'%(self.name,tagId, dataPointId)), 
            ...
            Base.AddRow([self.FormMgr.GetHtmlTable([tagId, 
                                               self.FormMgr.GetButton(session, 'del', 
                                                                  tgDivID, #Div Forms to process when pressed
                                                                  baseDiv, #Div to update when forms are processed.
            ...
            if 'childDataPoints' in self.pattern['tag'][tagId]:
                for parentDataPoint in self.pattern['tag'][tagId]['childDataPoints']:
                    ...
                    Base.AddRow([self.FormMgr.GetHtmlTable(['(dataPoint) %s'%(parentDataPoint), 
                                            self.FormMgr.GetButton(session, 'del',dptgDivID, baseDiv, 'Data__DataPattern__%s__dptag__%s__del'%(self.name,parentDataPoint))],dptgDivID, module), 
                                            TagDataPointsTb])
        ...
        try:
            ...
            pass
        try:
            AddTagTable = self.FormMgr.GetHtmlTable([GlobalTagTb, 'or',DPTagTb], newTgDivId, module) if len(dataPointIdTagList) > 0 else self.FormMgr.GetHtmlTable([GlobalTagTb, self.FormMgr.GetButton(session, 'add', newTgDivId, baseDiv, 'Data__DataPattern__%s__tag__new__add'%(self.name))], newTgDivId, module)
        except:
            AddTagTable = self.FormMgr.GetHtmlTable([GlobalTagTb, self.FormMgr.GetButton(session, 'add', newTgDivId, baseDiv, 'Data__DataPattern__%s__tag__new__add'%(self.name))], newTgDivId, module)
        try:
            if len(dataPointIdTagList) > 0:
                AddTagTable.AddRow(['', self.FormMgr.GetButton(session, 'add', newTgDivId,baseDiv, 'Data__DataPattern__%s__tag__new__add'%(self.name)), ''])
        except:
            pass
...
class DataPointConfigWiz(object):
    ...
    def GetHtmlOut(self, session):
        ...
        for item in self.ChunkerIns.Chunkdir:
            ...
            ChunkerDirHtml.AddRow([self.FormMgr.GetHtmlTable([self.ChunkerIns.Chunkdir[item], self.GetSplitTextBoxButton(session,self.ChunkerIns.ChunkList[item], msgChunkDivId)], msgChunkDivId)])
            chnkIndex+=1
    ...
    def GetSplitTextBoxButton(self, session, chunkRef, parentDiv = None):
        if chunkRef.isCluster: # Is a cluster Chunk
            ...
            for clusterItem in chunkRef.clusterList:
                ...
                ClusterSplitTable.AddRow([self.FormMgr.GetHtmlTable([clusterItem.string, self.GetSplitTextBoxButton(session,clusterItem, clusterDivId)], clusterDivId)])
                clusterInd+=1
            ...
            SplitTextBoxButtonTable = self.FormMgr.GetHtmlTable(['Split Text', 'Data Point ID'])
            ChunkChain = chunkRef.ChunkChain
            print (ChunkChain)
            TagList = []
            ...
            IdTextBoxButton= self.FormMgr.GetHtmlTable([HtmlTable([self.FormMgr.GetTextBox(session, 'IdTextTb_%s'%(iDString), ''), 
                                                   ...
                                                   self.FormMgr.GetButton(session, 'Add ID', splitIdTbDivId, baseDpDiv, '%s'%(iDString))])], splitIdTbDivId, 'ConfigWiz')
            SplitTextBoxButtonTable.AddRow([SplitTextBoxButton, IdTextBoxButton])    
            return SplitTextBoxButtonTable
