...
class Input:        
    ...
    def html(self):
        ...
        inputText = '<{eleType} type="{cName}"{value}{onclick}{prop}{name}>'.format(eleType=self.elementType, cName= self.cName if not self.cName == 'button' else 'submit',
                                                              prop=properties, value=' value="%s"'%(self.dispname) if not 'value' in self.kw else '',
                                                              name=' name="%s"'%(self.name) if 'name' in self.__dict__ else '',
                                                              onclick=' onclick=SendForm("%s","%s","%s") '%(self.kw['onclick']['id'], self.kw['onclick']['self'], self.kw['onclick']['tar']) if 'onclick' in self.kw else '') 
...
class navbar(htmlelement):
    ...
    def navprintOp(self):
        ...
        return retOps
    def html(self):
        toReturn = ['<nav class="navbar{options} {bg}">'.format(options=self.navprintOp(),
                                                            bg=self.bg),
                        '<div class="container-fluid">']
